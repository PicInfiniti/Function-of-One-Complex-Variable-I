...
\begin{proof}[Solution]
	We must construct a meromorphic function on $G$ with two kinds of prescribed local behavior:

	\begin{itemize}
		\item at each point $a_n$, the function must have a pole whose singular part is exactly the given principal part $S_n(z)$;
		\item at each point $b_m$, the function must have a zero of multiplicity exactly $p_m$.
	\end{itemize}
	...
	\noindent
	\textbf{Step 1: Construction of a meromorphic function with the prescribed singular parts.}
	...
	\begin{enumerate}
		\item the poles of $g$ are exactly the points $a_n$, and
		\item the singular part of $g$ at $a_n$ is $S_n(z)$ for each $n$.
	\end{enumerate}

	Thus, for each $n$, there exists a function $\phi_n$ holomorphic in a neighborhood of $a_n$ such that near $a_n$ we may write
	\[
		g(z)=S_n(z)+\phi_n(z).
	\]
	This means that $g$ already has precisely the desired pole behavior.

	Now observe that by hypothesis we have $a_n \neq b_m$ for all $n,m$. Hence none of the points $b_m$ is a pole of $g$. Therefore $g$ is holomorphic at every $b_m$, and all derivatives $g^{(k)}(b_m)$ are well-defined for every $k\ge 0$.

	\medskip

	\noindent
	\textbf{Step 2: Addition of a holomorphic correction term to create the prescribed zeros.}
	...
	\[
		F(b)=F'(b)=\cdots =F^{(p-1)}(b)=0
		\quad\text{and}\quad
		F^{(p)}(b)\neq 0.
	\]
	Applying this to $F=f$ and $b=b_m$, we see that we want
	\[
		f^{(k)}(b_m)=0 \qquad (0\le k\le p_m-1),
	\]
	and
	\[
		f^{(p_m)}(b_m)\neq 0.
	...
	\[
		h^{(k)}(b_m)=-g^{(k)}(b_m), \qquad 0\le k\le p_m-1,
	\]
	and also prescribe
	\[
	...
	\noindent
	\textbf{Step 3: Existence of the holomorphic correction term.}
	...
	\noindent
	\textbf{Step 4: Verification that $f$ has all the required properties.}
	...
	\noindent
	\emph{(i) Singular parts at the points $a_n$.}

	Since $h$ is holomorphic on all of $G$, it is holomorphic in a neighborhood of each $a_n$. Near $a_n$ we already know that
	\[
		g(z)=S_n(z)+\phi_n(z),
	\]
	where $\phi_n$ is holomorphic near $a_n$. Hence near $a_n$ we have
	\[
		f(z)=g(z)+h(z)=S_n(z)+\bigl(\phi_n(z)+h(z)\bigr).
	\]
	...
	\noindent
	\emph{(iii) Zeros at the points $b_m$ with multiplicity $p_m$.}

	For each $m$ and each $k$ with $0\le k\le p_m-1$, we have
	\[
		f^{(k)}(b_m)=g^{(k)}(b_m)+h^{(k)}(b_m)
		= g^{(k)}(b_m)-g^{(k)}(b_m)=0.
	\]
	Also,
	\[
		f^{(p_m)}(b_m)=g^{(p_m)}(b_m)+h^{(p_m)}(b_m)
		= g^{(p_m)}(b_m)+1-g^{(p_m)}(b_m)=1\neq 0.
	\]
	...
	\begin{itemize}
		\item its poles are exactly the points $a_n$;
		\item the singular part of $f$ at each $a_n$ is $S_n(z)$;
		\item each $b_m$ is a zero of $f$ of multiplicity exactly $p_m$.
	\end{itemize}
...
\begin{proof}[Solution]
	In general, the full conclusion of Theorem \(2.4\) is \emph{not} valid if \(f\) is only piecewise continuous on \(\partial D\).

	Indeed, if there were a function \(u:\overline D\to \mathbb R\) that is continuous on \(\overline D\), harmonic in \(D\), and satisfies
	\[
		u(\zeta)=f(\zeta)\qquad (\zeta\in \partial D),
	\]
	then the boundary function \(f=u|_{\partial D}\) would have to be continuous on \(\partial D\), since it is the restriction of a continuous function on \(\overline D\). Thus, if \(f\) has a jump discontinuity, such a \(u\) cannot exist.

	However, the Poisson integral still makes sense:
	\[
		u(re^{i\theta})
		=
		\frac1{2\pi}\int_{-\pi}^{\pi} P_r(\theta-t)\,f(e^{it})\,dt,
		\qquad 0\le r<1.
	\]
	Since a piecewise continuous function on \(\partial D\) is bounded and integrable, this formula defines a harmonic function \(u\) on \(D\).

	What remains true is the following:

	\begin{enumerate}
		\item The function \(u\) defined by the Poisson integral is harmonic in \(D\).

		\item At every point \(e^{i\theta_0}\in \partial D\) where \(f\) is continuous,
		      \[
			      \lim_{r\to 1^-} u(re^{i\theta_0}) = f(e^{i\theta_0}).
		      \]

		\item If \(e^{i\theta_0}\) is a jump discontinuity of \(f\), and the one-sided limits
		      \[
			      f(\theta_0^-),\qquad f(\theta_0^+)
		      \]
		      exist, then
	...
	So the theorem does \emph{not} remain true as stated: one generally loses continuity on \(\overline D\) and exact boundary agreement at the discontinuity points. What survives is the existence of a harmonic function in \(D\), given by the Poisson integral, which takes the correct boundary values at all continuity points and the average of the one-sided limits at each jump point.
\end{proof}
