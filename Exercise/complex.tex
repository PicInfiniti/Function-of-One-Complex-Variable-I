...
\begin{proof}[Solution]
	...
	\]
	Since \(f(G)\subset \Omega\) for every \(f\in\mathcal{F}\), and \(g\) is
	analytic on \(\Omega\), each function \(g\circ f\) is analytic on \(G\).
	Because \(\mathcal{F}\) is normal in \(H(G)\), Montel's theorem implies
	that \(\mathcal{F}\) is locally bounded. Fix \(a\in G\). Then there exist
	...
	\[
		|f(z)|\leq M
	\]
	for every \(f\in\mathcal{F}\) and every \(z\in B(a;r)\).

	Define
	\[
		E=\{f(z): f\in\mathcal{F},\ z\in B(a;r)\}.
	\]
	Then \(E\subset\Omega\), and \(E\) is bounded. Since \(g\) is bounded on
	bounded subsets of \(\Omega\), there exists \(N>0\) such that
	\[
		|g(w)|\leq N
	\]
	for every \(w\in E\). Hence, for every \(f\in\mathcal{F}\) and every
	\(z\in B(a;r)\),
	\[
		|(g\circ f)(z)|=|g(f(z))|\leq N.
	\]

	Therefore, the family \(\mathcal{G}\) is locally bounded on \(G\).
	By Montel's theorem, \(\mathcal{G}\) is normal in \(H(G)\).
\end{proof}
...
\end{exercise}
\begin{proof}[Solution]
	Suppose that \(f\) is not constant. We will show that \(f\) is one-one.

	Assume, to the contrary, that there exist distinct points \(z_1,z_2\in G\)
	such that
	\[
		f(z_1)=f(z_2).
	\]
	...
	\[
		g(z)=f(z)-f(z_1).
	\]
	Since \(f_n\to f\) uniformly on compact subsets of \(G\), and
	\(f_n(z_1)\to f(z_1)\), we have
	\[
		g_n\to g
	\]
	uniformly on compact subsets of \(G\).

	Because \(f\) is not constant and \(G\) is a region, the function \(g\)
	is not identically zero. Also,
	\[
		g(z_2)=f(z_2)-f(z_1)=0.
	\]
	Thus \(z_2\) is an isolated zero of \(g\). Choose \(r>0\) such that
	\[
		\overline{B(z_2;r)}\subset G,\qquad z_1\notin\overline{B(z_2;r)},
	\]
	and \(g\) has no zeros on \(\partial B(z_2;r)\).

	By Hurwitz's theorem, for all sufficiently large \(n\), the functions
	\(g_n\) and \(g\) have the same number of zeros in \(B(z_2;r)\), counting
	multiplicity. Since \(g\) has at least one zero there, \(g_n\) must also
	have a zero \(w_n\in B(z_2;r)\). Hence
	\[
		0=g_n(w_n)=f_n(w_n)-f_n(z_1),
	\]
	so
	\[
		f_n(w_n)=f_n(z_1).
	\]
	Since \(f_n\) is one-one, this implies \(w_n=z_1\), contradicting
	\(z_1\notin B(z_2;r)\).

	Therefore, if \(f\) is not constant, then \(f\) is one-one. Hence \(f\)
	is either one-one or constant.
\end{proof}
...
\end{exercise}
\begin{proof}[Solution]
	Since \(\Omega=f(G\setminus\{a\})\) is bounded, there exists \(M>0\)
	such that
	\[
		|f(z)|\leq M
	\]
	for every \(z\in G\setminus\{a\}\). In particular, \(f\) is bounded in
	a punctured neighborhood of \(a\). Hence, by Riemann's removable
	singularity theorem, \(f\) has a removable singularity at \(a\).

	Define
	\[
		f(a):=\lim_{z\to a} f(z).
	\]
	Then \(f\) extends to an analytic function on all of \(G\).
	Now suppose that \(f\) is one-to-one on \(G\setminus\{a\}\). Since
	\[
		\lim_{z\to a} f(z)=f(a),
	\]
	we have
	...
	We claim that \(f(a)\notin\Omega\). Suppose, for contradiction, that
	\(f(a)\in\Omega\). Then there exists \(z_0\in G\setminus\{a\}\) such that
	\[
		f(z_0)=f(a).
	\]

	Choose disjoint open neighborhoods \(U\) of \(a\) and \(V\) of \(z_0\),
	with \(U,V\subseteq G\). The extended function \(f\) is nonconstant
	because it is one-to-one on \(G\setminus\{a\}\). Therefore, by the open
	mapping theorem, \(f(U)\) and \(f(V)\) are open sets containing \(f(a)\).

	Thus, \(f(U)\cap f(V)\) is an open set containing \(f(a)\), so there
	exists
	\[
		w\in f(U)\cap f(V)
	\]
	such that \(w\neq f(a)\). Hence, there exist \(u\in U\) and \(v\in V\)
	such that
	\[
		f(u)=w=f(v).
	\]
	Since \(w\neq f(a)\), we have \(u\neq a\). Also, because
	\(U\cap V=\varnothing\), we have \(u\neq v\). Thus,
	\(u,v\in G\setminus\{a\}\) are distinct points satisfying
	\[
		f(u)=f(v),
	\]
	which contradicts the assumption that \(f\) is one-to-one.

	Therefore, \(f(a)\notin\Omega\). Since \(f(a)\in\overline{\Omega}\), it
	follows that
	\[
		f(a)\in\overline{\Omega}\setminus\Omega=\partial\Omega.
	\]
\end{proof}
