...
\end{exercise}
\begin{proof}[Solution]
	Let \(u:\Omega\to\mathbb{R}\) be a nonconstant harmonic function on a region
	\(\Omega\). We prove that \(u\) is an open map.
	Let \(G\subseteq\Omega\) be open, and let \(y_0\in u(G)\). Choose
	\(z_0\in G\) such that
	\[
		u(z_0)=y_0.
	\]
	Since \(G\) is open, there exists \(r>0\) such that
	\[
		\overline{B(z_0,r)}\subseteq G.
	\]

	We first observe that \(u\) is not constant on \(B(z_0,r)\). Indeed, if
	\(u\) were constant on \(B(z_0,r)\), then
	\[
		u_x-iu_y=0
	\]
	on \(B(z_0,r)\). Since \(u\) is harmonic, the function \(u_x-iu_y\) is
	analytic on \(\Omega\). By the identity theorem, it would vanish
	identically on \(\Omega\). Hence \(u_x=u_y=0\) on \(\Omega\), and since
	\(\Omega\) is connected, \(u\) would be constant on \(\Omega\), a
	contradiction.

	Therefore, by the maximum and minimum principles, \(u\) cannot attain a
	maximum or a minimum at the interior point \(z_0\). Thus there exist
	\(z_1,z_2\in B(z_0,r)\) such that
	\[
		u(z_1)<u(z_0)<u(z_2).
	\]

	The ball \(B(z_0,r)\) is connected, and \(u\) is continuous, so
	\(u(B(z_0,r))\) is a connected subset of \(\mathbb{R}\). Hence
	\(u(B(z_0,r))\) is an interval. Since this interval contains values both
	less than and greater than \(y_0=u(z_0)\), it contains an open interval
	about \(y_0\). More precisely, if
	\[
		\varepsilon
		=\frac12\min\{y_0-u(z_1),\,u(z_2)-y_0\}>0,
	\]
	then
	\[
	...
	Thus every point of \(u(G)\) is an interior point of \(u(G)\), so
	\(u(G)\) is open. Therefore \(u\) is an open map.
\end{proof}
