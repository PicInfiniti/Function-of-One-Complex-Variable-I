...
\begin{proof}[Solution (II)]
	...
	\[
		g'(w_0)\neq 0,
	\]
	the complex inverse function theorem implies that there exist neighborhoods
	$V$ of $w_0$ and $W$ of $g(w_0)$ such that
	\[
		g:V\to W
	\]
	...
	Since $f$ is continuous and $f(z_0)=w_0\in V$, there exists a neighborhood
	$U\subset G$ of $z_0$ such that
	\[
		f(U)\subset V.
	\]
	For every $z\in U$,
	...
	\[
		f=\phi\circ h.
	\]
	Since both $\phi$ and $h$ are analytic, $f$ is analytic on $U$.
	Because $z_0\in G$ was arbitrary, $f$ is analytic on $G$.
	Now differentiate
	\[
		h(z)=g(f(z)).
	\]
...
\begin{proof}[Solution]
	Let \(\Log\) denote the principal branch of the logarithm, which is analytic on
	\[
		\mathbb{C}\setminus(-\infty,0].
	\]
	Define
	\[
		f(z):=\exp\!\left(\frac12\Log(1-z^2)\right),
		\qquad
		g(z):=\exp\!\left(i\pi+\frac12\Log(1-z^2)\right).
	\]
	Thus
	\[
		g(z)=-f(z).
	...
	We first determine the domain on which these functions are defined. We require
	\[
		1-z^2\notin(-\infty,0].
		\]
		Now
		...
		1-z^2\in(-\infty,0]
			\iff z^2\in[1,\infty).
		\]
		If \(z^2\in[1,\infty)\), then \(z^2\) is a nonnegative real number, which
				forces \(z\) to be real. Since \(z^2\ge1\), we also have \(|z|\ge1\).
				Therefore
				\[
	...
	\[
		...
		\mathbb{C}\setminus\bigl((-\infty,-1]\cup[1,\infty)\bigr).
			...
			The set \(G\) is open and connected. In fact, it is star-shaped with respect
			to \(0\). Indeed, if \(z\in G\) and \(0\le t\le1\), then \(tz\) cannot lie on
			either of the removed rays unless \(z\) itself lies there.

			For \(z\in G\), we have
			\[
			1-z^2\in\mathbb{C}\setminus(-\infty,0],
	\]
	so \(\Log(1-z^2)\) is well-defined and analytic. Consequently, both \(f\) and
	\(g\) are analytic on \(G\), and therefore continuous.
	Moreover,
	\[
		f(z)^2
		=
		\exp\!\left(\Log(1-z^2)\right)
		=
		1-z^2,
	\]
	and
	\[
		g(z)^2
		=
		\exp\!\left(2i\pi+\Log(1-z^2)\right)
		=
		1-z^2.
	\]
	Thus
	...
	We now show that \(G\) is maximal with respect to this property.
	Suppose that there were a larger open connected set \(H\supsetneq G\) and a
	continuous function \(F:H\to\mathbb{C}\) satisfying
	\[
		F(z)^2=1-z^2
		\qquad (z\in H).
	\]
	Since \(1-z^2\neq0\) on \(G\), the quotient \(F/f\) is continuous on \(G\), and
	\[
		\left(\frac{F(z)}{f(z)}\right)^2=1.
	\]
	Hence
	\[
		\frac{F(z)}{f(z)}\in\{1,-1\}.
	\]
	Since \(G\) is connected and \(F/f\) is continuous, \(F/f\) must be constant.
	Therefore either
	\[
		F=f \quad\text{on }G,
	\]
	...
	We show that neither \(f\) nor \(-f\) can be continuously extended across any
	point of the removed rays.
	Take \(x>1\) and write \(z=x+iy\). Then
	\[
		1-z^2
		=
		1-x^2+y^2-2ixy.
	\]
	As \(y\to0^+\), the number \(1-z^2\) approaches the negative real number
	\(1-x^2\) from below. Hence its principal argument tends to \(-\pi\), and
	therefore
	\[
		f(z)
		\longrightarrow
		...
		-i\sqrt{x^2-1}.
	\]
	On the other hand, as \(y\to0^-\), the number \(1-z^2\) approaches the
	negative real axis from above, so its principal argument tends to \(\pi\).
	Thus
	\[
		f(z)
		\longrightarrow
		...
		i\sqrt{x^2-1}.
	\]
	These two limits are different, so \(f\) cannot be continuously extended
	across any \(x>1\). The same is true of \(-f\).
	The same argument applies to every point of
	\[
		(-\infty,-1).
	\]

	Finally, the points \(1\) and \(-1\) cannot be added either. Indeed, any open
	set containing \(1\) also contains points of \((1,\infty)\), and any open set
	containing \(-1\) also contains points of \((-\infty,-1)\). We have already
	shown that no continuous extension is possible across such points.
	Therefore there is no strictly larger open connected set containing \(G\) on
	which a continuous square root of \(1-z^2\) exists. Hence \(G\) is maximal.
	Thus a maximal choice is
	\[
	...
	\[
		...
		g(z)=-f(z)
	\]
	and both \(f\) and \(g\) are analytic on \(G\).
\end{proof}
...
\begin{proof}[Solution]
	Without loss of generality, assume that the point of tangency is \(a=0\), and that the centers of the two circles lie on the positive real axis. Let the outer circle have radius \(R\) and the inner circle have radius \(r\), where \(R>r>0\). Since both circles are tangent at \(0\), their centers are \(R\) and \(r\), respectively. Thus the circles are
	\[
		|z-R|=R
		\qquad\text{and}\qquad
		|z-r|=r.
	...
	Squaring gives
	\[
		|z|^2-2R\operatorname{Re} z=0.
	\]
	Dividing by \(|z|^2\), we obtain
	\[
		1-2R\operatorname{Re}\left(\frac{1}{z}\right)=0,
	\]
	so the outer circle is mapped to the vertical line
	\[
		\operatorname{Re} w=\frac{1}{2R}.
	\]
	Similarly, the inner circle is mapped to
	\[
		\operatorname{Re} w=\frac{1}{2r}.
	\]

	Therefore \(w=1/z\) maps \(G\) conformally onto the vertical strip
	\[
		\frac{1}{2R}<\operatorname{Re} w<\frac{1}{2r}.
	\]
	...
	The map
	\[
		u=\exp\left(\frac{\pi i(w-\alpha)}{\beta-\alpha}\right)
	\]
	maps this vertical strip conformally onto the upper half-plane. Indeed, if
	\(w=x+iy\), then
	\[
		\arg u=\frac{\pi(x-\alpha)}{\beta-\alpha},
	\]
	and since
	\[
		\alpha<x<\beta,
	...
	Finally, the Cayley transformation
	\[
		\zeta=\frac{u-i}{u+i}
	\]
	maps the upper half-plane conformally onto the open unit disk.
	Hence a conformal map from \(G\) onto the open unit disk is
	\[
		\zeta=
		\frac{
			\exp\left[
				\frac{\pi i}{\frac{1}{2r}-\frac{1}{2R}}
				\left(
				\frac{1}{z}-\frac{1}{2R}
				\right)
				\right]-i
...
\begin{proof}[Solution (II)]
	Suppose that $f$ is nonconstant. Since $G$ is a region and $f$ is analytic,
	the Open Mapping Theorem implies that $f(G)$ is open in $\mathbb{C}$.
	However, by hypothesis, $f(G)$ is contained in a circle, and a circle
	contains no nonempty open subset of $\mathbb{C}$. This is a contradiction.
	Hence $f$ is constant.
\end{proof}
...
\begin{proof}[Solution (I)]
	...
	\[
		\Omega=f(G).
	\]
	Since $f$ is one-to-one and analytic, Corollary 7.6 implies that
	\[
		f^{-1}:\Omega\to G
	\]
	is analytic.
	...
	\[
		(f^{-1})'(f(z))\,f'(z)=1.
	\]
	Hence $f'(z)$ cannot be zero. Therefore,
	\[
...
\end{exercise}
\begin{proof}[Solution]
	Suppose that \(f\) has no zeros in \(G\). We will show that \(f\) is constant.
	If \(c=0\), then \(|f(z)|=0\) for all \(z\in \partial G\). By the maximum modulus principle,
	\[
		|f(z)|\leq 0
	\]
	for all \(z\in G\). Hence \(f\equiv 0\), so \(f\) is constant.
	Now suppose that \(c>0\). Since \(f\) is nonzero in \(G\), by the minimum modulus principle,
	\[
		|f(z)|\geq c
	\]
	for all \(z\in G\). On the other hand, by the maximum modulus principle,
	\[
		|f(z)|\leq c
	\]
	for all \(z\in G\). Therefore,
	...
	\]
	for all \(z\in G\).
	Thus \(f(G)\) is contained in the circle
	\[
		\{w\in\mathbb{C}:|w|=c\}.
	\]
	If \(f\) were nonconstant, then by the open mapping theorem, \(f(G)\) would be open. But a circle is not an open subset of \(\mathbb{C}\). Hence \(f\) must be constant.
	Therefore, if \(f\) is not constant, it must have a zero in \(G\).
\end{proof}
...
\begin{proof}[Solution]
	By Proposition~1.10(b), it is enough to show that $\{f_n\}$ converges to $f$
	uniformly on every compact subset of $G$.

	Let $K \subset G$ be compact and let $\varepsilon>0$. Since $\{f_n\}$ is
	equicontinuous at each point of $G$, Proposition~1.22 implies that
	$\{f_n\}$ is equicontinuous on $K$. Hence, there exists $\delta_1>0$ such
	that
	\[
		z,w\in K,\qquad |z-w|<\delta_1
	\]
	implies
	\[
		d(f_n(z),f_n(w))<\frac{\varepsilon}{3}
	\]
	for every $n$.

	Since $f\in C(G,\Omega)$ and $K$ is compact, $f$ is uniformly continuous
	on $K$. Thus, there exists $\delta_2>0$ such that
	\[
		z,w\in K,\qquad |z-w|<\delta_2
	\]
	implies
	\[
		d(f(z),f(w))<\frac{\varepsilon}{3}.
	\]
	...
	\]
	By compactness of $K$, there exist points $z_1,\ldots,z_m\in K$ such that
	\[
		K\subset \bigcup_{j=1}^m B(z_j,\delta).
	\]

	For each $j=1,\ldots,m$, the pointwise convergence
	\[
		f_n(z_j)\longrightarrow f(z_j)
	\]
	implies that there exists $N_j\in\mathbb{N}$ such that
	\[
		n\geq N_j
	\]
	implies
	\[
		d(f_n(z_j),f(z_j))<\frac{\varepsilon}{3}.
	\]
	Let
	\[
		N=\max\{N_1,\ldots,N_m\}.
	...
	Now let $n\geq N$ and $z\in K$. Choose $j$ such that
	$|z-z_j|<\delta$. Then, by the triangle inequality,
	\[
		\begin{aligned}
			d(f_n(z),f(z))
			 & \leq d(f_n(z),f_n(z_j))
	...
	Therefore,
	\[
		\sup_{z\in K} d(f_n(z),f(z))\longrightarrow 0.
	\]

	Thus, $f_n\to f$ uniformly on every compact subset of $G$. By
	Proposition~1.10(b), it follows that
	\[
		f_n\longrightarrow f
	\]
	in $C(G,\Omega)$.
\end{proof}\setexercise{8}
\begin{exercise}
	\begin{enumerate}[label=(\alph*)]
...
\begin{proof}[Solution]
	First suppose that \(\mathcal{F}\) is normal. Let
	\[
		\{h_n\}\subset \mathcal{F}'.
	\]
	For each \(n\), choose \(f_n\in\mathcal{F}\) such that \(h_n=f_n'\).
	Since \(\mathcal{F}\) is normal, there is a subsequence
	\(\{f_{n_k}\}\) and a function \(f\in H(G)\) such that
	\[
		f_{n_k}\longrightarrow f
	\]
	uniformly on compact subsets of \(G\). Convergence in \(H(G)\) implies
	convergence of derivatives, so
	\[
		h_{n_k}=f_{n_k}'\longrightarrow f'
	\]
	uniformly on compact subsets of \(G\). Thus every sequence in
	\(\mathcal{F}'\) has a convergent subsequence, and therefore
	\(\mathcal{F}'\) is normal.

	The converse is not true. For example, let
	\[
		\mathcal{F}=\{f_n:n\geq 1\},
		\qquad
		f_n(z)=n.
	\]
	Then
	\[
		\mathcal{F}'=\{0\},
	\]
	which is normal. However, \(\mathcal{F}\) is not locally bounded and
	hence is not normal.
	A sufficient additional hypothesis is that, for some fixed
	\(a\in G\),
	\[
		\{f(a):f\in\mathcal{F}\}
	\]
	is bounded. Assume that \(G\) is a region, that \(\mathcal{F}'\) is
	normal, and that
	\[
		\sup_{f\in\mathcal{F}}|f(a)|<\infty
	\]
	for some \(a\in G\). We show that \(\mathcal{F}\) is normal.

	Let \(\{f_n\}\subset\mathcal{F}\). Since \(\mathcal{F}'\) is normal,
	there is a subsequence, which we continue to denote by \(\{f_n\}\),
	such that
	\[
		f_n'\longrightarrow h
	\]
	uniformly on compact subsets of \(G\), for some \(h\in H(G)\).
	The sequence \(\{f_n(a)\}\) is bounded, so, after passing to a further
	subsequence, we may assume that
	\[
		f_n(a)\longrightarrow c
	\]
	for some \(c\in\mathbb{C}\).

	Fix \(z_0\in G\). Since \(G\) is a region, there is a piecewise smooth
	path \(\gamma\) in \(G\) joining \(a\) to \(z_0\). Choose \(r>0\) such
	that
	\[
	...
	Set
	\[
		L=\gamma([0,1])\cup\overline{B(z_0;r)}.
	\]
	Then \(L\) is a compact subset of \(G\). For \(z\in\overline{B(z_0;r)}\),
	the fundamental theorem for complex line integrals gives
	\[
		f_n(z)-f_m(z)
		=
		f_n(a)-f_m(a)
		+\int_{\gamma}(f_n'-f_m')(w)\,dw
		+\int_{[z_0,z]}(f_n'-f_m')(w)\,dw.
	\]
	Consequently,
	\[
		|f_n(z)-f_m(z)|
		\leq
		|f_n(a)-f_m(a)|
		+\bigl(\ell(\gamma)+r\bigr)
		\sup_{w\in L}|f_n'(w)-f_m'(w)|.
	\]
	The right-hand side tends to zero as \(m,n\to\infty\), independently
	of \(z\in\overline{B(z_0;r)}\). Thus \(\{f_n\}\) is uniformly Cauchy
	on a neighborhood of every point of \(G\).

	By covering an arbitrary compact subset of \(G\) by finitely many such
	neighborhoods, it follows that \(\{f_n\}\) is uniformly Cauchy on every
	compact subset of \(G\). Since \(H(G)\) is complete, \(\{f_n\}\)
	converges in \(H(G)\).
	Therefore every sequence in \(\mathcal{F}\) has a convergent
	subsequence, so \(\mathcal{F}\) is normal.
\end{proof}
...
\end{exercise}
\begin{proof}[Solution]
	Since \(\mathcal{F}\) is normal in \(H(G)\), it is locally bounded. Thus, for every compact set \(K\subset G\), there exists a constant \(M>0\) such that
	\[
		|f(z)|\leq M
	\]
	...
	Hence
	\[
		f(K)\subset \Omega\cap\{w\in\mathbb{C}:|w|\leq M\}
	\]
	for every \(f\in\mathcal{F}\). Since \(g\) is bounded on bounded sets, there exists a constant \(C>0\) such that
	\[
		|g(w)|\leq C
	\]
	for all
	\[
		w\in \Omega\cap\{w\in\mathbb{C}:|w|\leq M\}.
	\]
	Therefore,
	\[
		|g(f(z))|\leq C
	...
	\]
	is locally bounded on \(G\). Since each \(g\circ f\) is analytic on \(G\), Montel's theorem implies that
	\[
		\{g\circ f:f\in\mathcal{F}\}
	\]
	is normal in \(H(G)\).
...
\end{exercise}
\begin{proof}[Solution]
	Recall that convergence in \(H(D)\) means uniform convergence on compact
	subsets of \(D\).
	Suppose first that \(f_n \to f\) in \(H(D)\). For any \(0<r<1\), the
	circle \(\gamma_r\) is a compact subset of \(D\). Hence,
	\[
		\sup_{|z|=r}|f(z)-f_n(z)|\longrightarrow 0.
	\]
	Since the length of \(\gamma_r\) is \(2\pi r\), we have
	\[
		\int_{\gamma_r}|f(z)-f_n(z)|\,|dz|
		\leq 2\pi r\sup_{|z|=r}|f(z)-f_n(z)|
		\longrightarrow 0.
	\]

	Conversely, suppose that
	\[
		\int_{\gamma_r}|f(z)-f_n(z)|\,|dz|\longrightarrow 0
	\]
	for every \(0<r<1\). Let \(K\subset D\) be compact. Choose numbers
	\(0<\rho<r<1\) such that
	\[
		K\subset \overline{B(0;\rho)}.
	\]
	For \(z\in \overline{B(0;\rho)}\), Cauchy's integral formula applied to
	the analytic function \(f-f_n\) gives
	\[
		f(z)-f_n(z)
		=
		\frac{1}{2\pi i}
		\int_{\gamma_r}
		\frac{f(w)-f_n(w)}{w-z}\,dw.
	\]
	Since \(|w|=r\) and \(|z|\leq \rho\), we have
	\[
		|w-z|\geq r-\rho.
	\]
	Therefore,
	\[
		...
		\leq
		\frac{1}{2\pi(r-\rho)}
		\int_{\gamma_r}|f(w)-f_n(w)|\,|dw|.
	\]
	Taking the supremum over \(z\in K\), we obtain
	\[
		\sup_{z\in K}|f(z)-f_n(z)|
		\leq
		\frac{1}{2\pi(r-\rho)}
		...
		\longrightarrow 0.
	\]
	Thus \(f_n\to f\) uniformly on every compact subset of \(D\), and hence
	\(f_n\to f\) in \(H(D)\).
\end{proof}
...
\end{exercise}
\begin{proof}[Solution]
	Suppose that $f$ is not one-one. Then there exist distinct points
	$a,b\in G$ such that
	\[
		f(a)=f(b).
	\]
	...
	\[
		g_n(z)=f_n(z)-f_n(a).
	\]
	Since $f_n\to f$ uniformly on compact subsets of $G$, we have
	\[
		g_n(z)\to g(z):=f(z)-f(a)
	\]
	uniformly on compact subsets of $G$.
	Because each $f_n$ is one-one,
	\[
		g_n(z)=0
		\iff f_n(z)=f_n(a)
		\iff z=a.
	\]
	Thus $g_n$ has no zeros in $G\setminus\{a\}$.
	Since $a\neq b$, choose $r>0$ such that
	\[
		\overline{D(b,r)}\subset G
		\qquad\text{and}\qquad
		a\notin \overline{D(b,r)}.
	\]
	Hence each $g_n$ is zero-free on $D(b,r)$. On the other hand,
	\[
		g(b)=f(b)-f(a)=0.
	\]
	By Hurwitz's theorem, since the $g_n$ are zero-free on $D(b,r)$ and
	converge uniformly on compact subsets to $g$, either $g$ is zero-free
	on $D(b,r)$ or $g\equiv 0$ there. Since $g(b)=0$, it follows that
	\[
		g\equiv 0 \quad \text{on } D(b,r).
	\]
	Therefore,
	\[
		f(z)=f(a)
	\]
	for all $z\in D(b,r)$. Since $G$ is connected, the identity theorem
	implies that
	\[
		f(z)\equiv f(a)
	\]
	on all of $G$. Thus $f$ is constant.
	Hence, if $f$ is not constant, it must be one-one. Therefore either
	$f$ is one-one or $f$ is constant.
\end{proof}
...
\begin{proof}[Solution]
	\begin{enumerate}[label=(\alph*)]
		...
		\item Since $\Omega=f(G\setminus\{a\})$ is bounded, there exists $M>0$ such that
		      \[
			      |f(z)|\le M
		      \]
		      for all $z\in G\setminus\{a\}$. Hence $f$ is bounded in a punctured neighborhood of $a$. By the Riemann removable singularity theorem, $f$ has a removable singularity at $a$. Thus $f$ extends to an analytic function on all of $G$, which we continue to denote by $f$.
		      Now suppose that $f$ is one-one on $G\setminus\{a\}$. Since $f$ is continuous at $a$, if $z_n\to a$ with $z_n\neq a$, then
		      \[
			      f(z_n)\to f(a).
		      \]
		      Since $f(z_n)\in\Omega$, it follows that
		      \[
			      f(a)\in\overline{\Omega}.
		      \]

		      We claim that $f(a)\notin\Omega$. Suppose, to the contrary, that $f(a)\in\Omega$. Since
		      \[
			      \Omega=f(G\setminus\{a\}),
		      \]
		      there exists $b\in G\setminus\{a\}$ such that
		      \[
			      f(b)=f(a).
		      \]
		      Choose disjoint neighborhoods $U$ of $a$ and $V$ of $b$ contained in $G$. Since $f$ is nonconstant and analytic, the open mapping theorem implies that $f(U)$ and $f(V)$ are open. Both contain $f(a)$, so their intersection contains some
		      \[
			      w\neq f(a).
		      \]
		      Thus there exist $u\in U$ and $v\in V$ such that
		      \[
			      f(u)=w=f(v).
		      \]
		      Since $w\neq f(a)$, we have $u\neq a$, and since $U\cap V=\varnothing$, we have $u\neq v$. This contradicts the assumption that $f$ is one-one on $G\setminus\{a\}$.
		      Therefore
		      \[
			      f(a)\notin\Omega.
		      \]
		      Since $f(a)\in\overline{\Omega}$, we conclude that
		      \[
			      f(a)\in\partial\Omega
		      \]

		\item Suppose, toward a contradiction, that there exists a one-one analytic function
		      \[
			      f:\{z:0<|z|<1\}\longrightarrow \Omega
		      \]
		      onto
		      \[
			      \Omega=\{w:r<|w|<R\},
			      \qquad r>0.
		      \]
		      Since $\Omega$ is bounded, part (a) implies that the singularity of $f$ at $0$ is removable. Hence $f$ extends analytically to the disk
		      \[
			      D=\{z:|z|<1\}.
		      \]
		      Moreover, by part (a),
		      ...
		      Since $f$ maps the punctured disk onto $\Omega$, we have
		      \[
			      f(D)=\Omega\cup\{f(0)\}.
		      \]
		      But $f(0)\in\partial\Omega$, so the set
		      \[
			      \Omega\cup\{f(0)\}
		      \]
		      is not open. On the other hand, $f$ is a nonconstant analytic function on $D$, so by the open mapping theorem $f(D)$ must be open. This is a contradiction.
		      Therefore there is no one-one analytic function mapping
		      \[
			      \{z:0<|z|<1\}
		      \]
		      onto an annulus
...
\begin{proof}[Solution]
	Write the prescribed singular part at \(a_n\) as
	\[
		S_n(z)=\sum_{k=1}^{q_n}\frac{c_{n,k}}{(z-a_n)^k},
		\qquad c_{n,q_n}\neq 0,
	\]
	where \(q_n\) is the order of the pole prescribed at \(a_n\).
	By the Weierstrass theorem, there exist holomorphic functions \(A\) and
	\(B\) on \(G\) such that
	\[
		\operatorname{ord}_{a_n}A=q_n,
		\qquad
		\operatorname{ord}_{b_m}B=p_m,
	\]
	and \(A\) and \(B\) have no other zeros. Since \(a_n\neq b_m\) for all
	\(n,m\), the function
	\[
		F=\frac{B}{A}
	\]
	is meromorphic on \(G\), has poles precisely at the points \(a_n\), and
	has zeros precisely at the points \(b_m\), with multiplicity \(p_m\).
	Near \(a_n\), write
	\[
		F(z)=\frac{C_n(z)}{(z-a_n)^{q_n}},
	\]
	where \(C_n\) is holomorphic near \(a_n\) and \(C_n(a_n)\neq 0\). Also,
	write
	\[
		S_n(z)=\frac{P_n(z)}{(z-a_n)^{q_n}},
	\]
	where
	\[
		P_n(z)
		...
		+c_{n,1}(z-a_n)^{q_n-1}.
	\]
	Since \(P_n(a_n)=c_{n,q_n}\neq 0\), the function
	\[
		U_n(z)=\frac{P_n(z)}{C_n(z)}
	\]
	is holomorphic and nonzero in a neighborhood of \(a_n\). Hence there
	exists a holomorphic function \(L_n\) near \(a_n\) such that
	\[
		e^{L_n}=U_n.
	\]

	We now construct a holomorphic function \(g\) on \(G\) satisfying
	\[
		g(z)-L_n(z)=O\bigl((z-a_n)^{q_n}\bigr)
	\]
	near every \(a_n\).

	Indeed, by the Weierstrass theorem, choose a holomorphic function
	\(\phi\) on \(G\) having a zero of order \(q_n\) at each \(a_n\) and no
	other zeros. For every \(n\), let \(Q_n\) be the principal part at
	\(a_n\) of the meromorphic function
	\[
		\frac{L_n}{\phi}.
	\]
	By the Mittag--Leffler theorem, there exists a meromorphic function
	\(M\) on \(G\) whose principal part at \(a_n\) is \(Q_n\). Then
	\[
		g=\phi M
	\]
	is holomorphic on \(G\). Moreover, near \(a_n\),
	\[
		g-L_n
		=
		\phi\left(M-\frac{L_n}{\phi}\right)
		=
		O\bigl((z-a_n)^{q_n}\bigr).
	\]
	...
	\[
		f=e^gF.
	\]
	Since \(e^g\) is holomorphic and nowhere zero, \(f\) has the same poles
	and zeros as \(F\). Thus its poles are precisely the points \(a_n\), and
	its zeros are precisely the points \(b_m\), with \(b_m\) having
	multiplicity \(p_m\).

	Finally, since
	\[
		g-L_n=O\bigl((z-a_n)^{q_n}\bigr),
	\]
	we have
	\[
		e^g-e^{L_n}
		...
		O\bigl((z-a_n)^{q_n}\bigr).
	\]
	Using \(e^{L_n}=U_n=P_n/C_n\), we obtain near \(a_n\)
	\[
		\begin{aligned}
			f(z)-S_n(z)
			 & =
			\frac{C_n(z)e^{g(z)}-P_n(z)}
			{(z-a_n)^{q_n}} \\
			 & =
			\frac{C_n(z)\bigl(e^{g(z)}-e^{L_n(z)}\bigr)}
			{(z-a_n)^{q_n}}.
		\end{aligned}
	\]
	The last expression is holomorphic at \(a_n\). Therefore
	\(f-S_n\) is holomorphic at \(a_n\), so the singular part of \(f\) at
	\(a_n\) is exactly \(S_n\).

	Hence \(f\) has all the required properties.
\end{proof}
...
\begin{proof}[Solution]
	Let \(u:G\to\mathbb{R}\) be a nonconstant harmonic function. We will show that \(u\) is an open map.

	Let \(U\subset G\) be open, and let \(z_0\in U\). Since \(U\) is open, there exists \(r>0\) such that
	\[
		D(z_0,r)\subset U.
	\]

	Since \(u\) is a nonconstant harmonic function, the maximum and minimum principles imply that \(u\) cannot have a local maximum or a local minimum at \(z_0\). Hence there exist points \(z_1,z_2\in D(z_0,r)\) such that
	\[
		u(z_1)<u(z_0)<u(z_2).
	\]

	The disk \(D(z_0,r)\) is connected, and \(u\) is continuous. Therefore
	\[
		u(D(z_0,r))
	\]
	is a connected subset of \(\mathbb{R}\). Since the connected subsets of \(\mathbb{R}\) are intervals, \(u(D(z_0,r))\) is an interval.

	Since
	\[
		u(z_1)<u(z_0)<u(z_2),
	\]
	the interval \(u(D(z_0,r))\) contains
	\[
		\bigl(u(z_1),u(z_2)\bigr).
	\]
	Thus \(u(z_0)\) is an interior point of \(u(D(z_0,r))\). Since
	\[
		u(D(z_0,r))\subset u(U),
	\]
	it follows that \(u(z_0)\) is an interior point of \(u(U)\).

	Since \(z_0\in U\) was arbitrary, every point of \(u(U)\) is an interior point. Hence \(u(U)\) is open in \(\mathbb{R}\). Therefore \(u\) is an open map.
\end{proof}
...
\begin{proof}[Solution]
	...
	\[
		u_{xx}+u_{yy}=0.
	\]
	We claim that \(f\) is holomorphic on \(G\). Indeed, writing
	\[
		f=u_x-i u_y,
	\]
	...
	\[
		...
		u_{xy}=u_{yx},
	\]
	which hold because \(u\) is harmonic and has continuous second partial
	derivatives.
	Now
	\[
		f(z)=0
	...
	\[
		A=\{z\in G:u_x(z)=u_y(z)=0\}
	\]
	is precisely the set of zeros of the holomorphic function \(f\).
	Suppose that \(A\) has a limit point in \(G\). Then \(f\) has a set of
	zeros with a limit point in \(G\). By the Identity Theorem,
	\[
		f\equiv 0
	\]
	on \(G\). Hence
	\[
		u_x=u_y=0
	\]
	throughout \(G\). Since \(G\) is a region and therefore connected, \(u\)
	must be constant on \(G\).
	Therefore, if \(u\) is nonconstant, \(A\) cannot have a limit point in
	\(G\). If \(u\) is constant, then
	\[
		A=G,
	\]
	so \(A\) does have limit points in \(G\). Thus \(A\) can have a limit
	point in \(G\) if and only if \(u\) is constant.
\end{proof}
